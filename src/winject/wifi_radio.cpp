...
// Firmware builds the 802.11 header. Same-object call in libnet80211, so
// --wrap cannot intercept; muldefs overrides the sanity check.
extern "C" int ieee80211_raw_frame_sanity_check(int32_t, int32_t, uint32_t, uint32_t) {
    return 0;
}
...
static volatile uint32_t g_wifiRx = 0;
static volatile uint32_t g_wifiRxDropped = 0;
...
static void formatApSsid(char *out, size_t outLen) {
    uint8_t mac[6] = {};
    WiFi.macAddress(mac);
    snprintf(out, outLen, "winject-%02X%02X%02X%02X%02X%02X", mac[0], mac[1], mac[2],
             mac[3], mac[4], mac[5]);
}
...
static bool applyChannel() {
    ...
    if (err != ESP_OK) {
        Serial.printf("wifi set_channel %u failed: %s\n", g_channel, esp_err_to_name(err));
        return false;
    }
    return true;
}
...
static bool applyModulation() {
    ...
    if (err != ESP_OK) {
        Serial.printf("wifi set_protocol failed: %s\n", esp_err_to_name(err));
        return false;
    }
    ...
    // Rate config is documented as pre-start; reapply around a restart.
    err = esp_wifi_stop();
    if (err != ESP_OK) {
        Serial.printf("wifi stop for rate failed: %s\n", esp_err_to_name(err));
        return false;
    }
    err = esp_wifi_config_80211_tx_rate(WIFI_IF_STA, g_modulationRate);
    if (err != ESP_OK) {
        Serial.printf("wifi config_80211_tx_rate failed: %s\n", esp_err_to_name(err));
        esp_wifi_start();
        return false;
    }
    err = esp_wifi_start();
    if (err != ESP_OK) {
        Serial.printf("wifi start after rate failed: %s\n", esp_err_to_name(err));
        return false;
    }
    return true;
}
...
static bool applyMonitor() {
    ...
    esp_wifi_set_promiscuous_rx_cb([](void *buf, wifi_promiscuous_pkt_type_t type) {
        if (type == WIFI_PKT_MISC || buf == nullptr || g_rxFree == nullptr) {
            return;
        }
...
static bool applyCca() {
    // MAC TX CCA (CSMA wait-for-idle). PHY CCA disable, if present, is
    // additional and can prevent reception on some ESP32 PHY revisions.
    const int err = hal_mac_tx_set_cca(g_ccaEnabled ? 1 : 0);
    if (err != 0) {
        Serial.printf("wifi hal_mac_tx_set_cca failed: %d\n", err);
        return false;
    }
...
bool wifiRadioBegin() {
    ...
    if (g_rxFree == nullptr || g_rxFilled == nullptr) {
        Serial.println("wifi rx queue alloc failed");
        return false;
    }
    ...
    if (g_apActive) {
        Serial.println("wifi radio: AP fallback already active");
        return false;
    }
    ...
    g_radioReady = true;
    Serial.printf("wifi monitor/inject on channel %u rate %s\n", g_channel, g_modulationName);
    return true;
}
...
bool wifiRadioStartApFallback() {
    ...
    const bool apOk = WiFi.softAP(g_apSsid, nullptr, apChannel, 0, WIFI_AP_MAX_CLIENTS, false,
                                  WIFI_AUTH_OPEN);
#else
    const bool apOk = WiFi.softAP(g_apSsid, nullptr, apChannel, 0, WIFI_AP_MAX_CLIENTS);
#endif
    if (!apOk) {
    ...
    g_apActive = true;
    Serial.printf("wifi AP fallback SSID %s  IP %s  (open)%s\n", g_apSsid,
...
bool wifiRadioApIpv4(uint32_t *out) {
    if (out == nullptr || !g_apActive) {
        return false;
    }
    *out = static_cast<uint32_t>(WiFi.softAPIP());
    return *out != 0;
}
...
bool wifiRadioInject(const uint8_t *frame, size_t len) {
    ...
    const esp_err_t err = esp_wifi_80211_tx(WIFI_IF_STA, frame, static_cast<int>(len), false);
    if (err != ESP_OK) {
        g_wifiTxFail++;
        return false;
    }
...
void wifiRadioGetStatus(WifiRadioStatus *status) {
    ...
    status->wifiRx = g_wifiRx;
    status->wifiTx = g_wifiTx;
    status->wifiRxDropped = g_wifiRxDropped;

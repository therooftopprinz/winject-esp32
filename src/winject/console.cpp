...
static void printMac(const ConsoleOut &out, const uint8_t mac[6]) {
    consolePrintf(out, "%02X:%02X:%02X:%02X:%02X:%02X", mac[0], mac[1], mac[2], mac[3],
                  mac[4], mac[5]);
}
...
static void printHelp(const ConsoleOut &out) {
    ...
    consoleWrite(out, "\n");
    consoleWrite(out, "ota: ArduinoOTA UDP 3232, HTTP POST /update\n");
}
...
static void printStatus(const ConsoleOut &out) {
    ...
    if (otaActive()) {
        consolePrintf(out, "ota 3232  http %u/update\n", OTA_HTTP_PORT);
    } else {
        consoleWrite(out, "ota waiting for ipv4\n");
    }
    ...
    consolePrintf(out, "wifi_rx %u  wifi_tx %u  wifi_rx_drop %u  wifi_tx_fail %u\n",
...
static void feedChar(char c, char *line, size_t *len, size_t maxLen, const ConsoleOut &out) {
    if (c == '\r') {
        return;
    }
    if (c == '\n') {
...
static void consoleStartTcp() {
    ...
    if (fd < 0) {
        Serial.printf("console socket failed: %d\n", errno);
        return;
    }
    ...
    if (bind(fd, reinterpret_cast<struct sockaddr *>(&addr), sizeof(addr)) < 0) {
        Serial.printf("console bind *:%u failed: %d\n", CONTROL_CONSOLE_PORT, errno);
        close(fd);
        return;
    }
    if (listen(fd, 2) < 0 || !setNonblock(fd)) {
        Serial.printf("console listen failed: %d\n", errno);
        close(fd);
        return;
    }
    ...
    if (ethernetLocalIpv4(&ip)) {
        ...
        Serial.printf("control console on %s:%u\n", ipStr, CONTROL_CONSOLE_PORT);
    }
    if (wifiRadioApIpv4(&ip)) {
        char ipStr[16];
        ipv4ToString(ip, ipStr, sizeof(ipStr));
        Serial.printf("control console on %s:%u (ap)\n", ipStr, CONTROL_CONSOLE_PORT);
    }
}
...
static void acceptClient() {
    ...
    const int fd = accept(g_listenFd, reinterpret_cast<struct sockaddr *>(&peer), &peerLen);
    if (fd < 0) {
        return;
    }
...
void consolePoll() {
    ...
    while (Serial.available() > 0) {
        feedChar(static_cast<char>(Serial.read()), g_serialLine, &g_serialLen,

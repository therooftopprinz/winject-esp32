...
static void forwardRx()
{
  ...
  while (wifiRadioPopRx(g_buf, &len, sizeof(g_buf)))
  {
    ...
    const int sent =
        sendto(g_udpFd, payload, payloadLen, 0,
...
static void upstreamTask(void* arg)
{
  (void)arg;
  for (;;)
  {
...
void upstreamBegin()
{
  ...
  xTaskCreatePinnedToCore(upstreamTask, "upstream", 6144, nullptr,
                          UPSTREAM_TASK_PRIO, nullptr, APP_TASK_CORE);
}

bool upstreamSetRxPort(uint16_t port)
{
  if (port == 0 || g_lock == nullptr)
  {
    return false;
  }
  xSemaphoreTake(g_lock, portMAX_DELAY);
  ...
  g_rxPort = port;
  xSemaphoreGive(g_lock);
  ESP_LOGI(TAG, "rx listening on UDP %u", port);
  return true;
}

bool upstreamSetTx(uint32_t host, uint16_t port)
{
  if (port == 0 || host == 0 || host == 0xFFFFFFFFu || g_lock == nullptr)
  {
    return false;
  }
  xSemaphoreTake(g_lock, portMAX_DELAY);
  ...
  ESP_LOGI(TAG, "tx %s:%u", ipStr, port);
  return true;
}

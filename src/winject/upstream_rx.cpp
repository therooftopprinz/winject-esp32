...
void upstream_rx::publish_local_airports()
{
    ...
    for (size_t i = 0; i < WIFI_AIRPORT_MAX; i++)
    {
        if (rx_[i].used)
        {
            memcpy(local_airports_[n_local], rx_[i].airport, 6);
            n_local++;
        }
    }
    frameSetLocalAirports(local_airports_, n_local);
}
...
bool upstream_rx::set(const uint8_t airport[6], uint16_t port)
{
    ...
    if (find_port(port, idx) >= 0)
    {
        ESP_LOGE(TAG, "udp port %u already bound", port);
        return false;
    }

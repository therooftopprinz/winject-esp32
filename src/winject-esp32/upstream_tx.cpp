...
bool upstream_tx::open_send(bfc::socket* out)
{
    ...
    // INADDR_ANY: do not call eth_->local_ipv4() here (can fail/block under
    // lock_ and then the RX loop drops every popped frame).
...
{
    uint8_t peer_airports[WIFI_AIRPORT_MAX][6];
    size_t n_peer = 0;
    for (size_t i = 0; i < WIFI_AIRPORT_MAX; i++)
    {
        if (tx_[i].used)
        {
            memcpy(peer_airports[n_peer], tx_[i].airport, 6);
            n_peer++;
        }
    }
    frameSetPeerAirports(peer_airports, n_peer);
}
                               ...
                               uint32_t host, uint16_t port)
{
    ...
    if (n < 0 && errno != EAGAIN && errno != EWOULDBLOCK)
    {
        ...
        ESP_LOGW(TAG, "udp send %s:%u failed: %d", ip_str, port, errno);
    }
}

void upstream_tx::forward_mpdu(const uint8_t* mpdu, size_t len)
{
    const uint8_t* payload = nullptr;
    size_t payload_len = 0;
    if (mpdu == nullptr || !frameUnwrap(mpdu, len, &payload, &payload_len))
    {
        return;
    }
    ...
    for (size_t i = 0; i < WIFI_AIRPORT_MAX; i++)
    {
        if (tx_[i].used && frameStandaloneSaMatchesAirport(sa, tx_[i].airport))
        {
            send_payload(payload, payload_len, tx_[i].host, tx_[i].port);
            break;
        }
...
void upstream_tx::forward_rx()
{
    ...
    size_t len = 0;
    while (wifi::instance().pop_rx(buf_, &len, sizeof(buf_), 0))
    {
        forward_mpdu(buf_, len);
    }
}
...
void upstream_tx::run()
{
    for (;;)
    {
        ...
        size_t len = 0;
        if (!wifi::instance().pop_rx(buf_, &len, sizeof(buf_),
                                     pdMS_TO_TICKS(20)))
        {
            continue;
        }
...
bool upstream_tx::unset(const uint8_t airport[6])
{
    ...
    tx_[idx].host = 0;
    tx_[idx].port = 0;
    memset(tx_[idx].airport, 0, sizeof(tx_[idx].airport));
    publish_peer_airports();
    return true;
}

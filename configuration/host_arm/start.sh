#!/usr/bin/env bash
# ARM video source (camera): winject-manager + GStreamer UVC encode.
# Default: RTP/UDP.  ./start.sh --tcp-gst  → MPEG-TS/UDP (see gst.env ports).
# Both manager upstreams are UDP_SERVER; manager binds before gst (udpsink).
set -euo pipefail
...
if [[ "$TRANSPORT" == tcp ]]; then
    PORT="${PORT:-${TCP_PORT:-22082}}"
else
    PORT="${PORT:-${UDP_PORT:-22081}}"
fi
PROTO=UDP
SS_FLAG=-uln
...
ensure_winject_manager "$ROOT" build_manager_arm
...
for _ in $(seq 1 50); do
    if ! kill -0 "$MGR_PID" 2>/dev/null; then
        wait "$MGR_PID" || true
        echo "winject-manager exited before binding ${PROTO} ${PORT}" >&2
        exit 1
    fi
...
if [[ "$ready" -ne 1 ]]; then
    echo "winject-manager did not bind ${PROTO} ${PORT} in time" >&2
    exit 1
fi
